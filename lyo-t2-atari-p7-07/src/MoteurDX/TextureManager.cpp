...
bool TextureManager::LoadTexture(const std::wstring& id, const std::wstring& filename)
{
    ...
    // Le Shader Resource View (SRV) est un descripteur qui dÈfinit comment une ressource (ici une texture) est vue par les shaders. Il prÈcise notamment 
    // le format de la texture, le nombre de mipmaps et la dimension de la ressource. Gr‚ce au SRV, le shader (souvent le pixel shader) peut Èchantillonner 
    // la texture pour en extraire les donnÈes de couleur lors du rendu.
...
void TextureManager::CreateDescriptorHeap(UINT numDescriptors)
{
    ...
    // Le descriptor heap est une zone mÈmoire rÈservÈe sur la carte graphique qui stocke plusieurs descripteurs. Dans le cas du SRV, il contient les 
    // descriptions de ressources (comme les textures) afin que le GPU puisse y accÈder directement lors du rendu. Une fois crÈÈ et rempli, le heap permet 
    // de regrouper et de gÈrer ces descripteurs de faÁon efficace et de les rendre visibles aux shaders.
