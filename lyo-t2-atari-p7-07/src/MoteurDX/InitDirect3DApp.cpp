...
//
//}
bool InitDirect3DApp::AABBIntersect(TransformComponent& a, TransformComponent& b)
{
...
void InitDirect3DApp::UpdatePhysics()
{
	...
	for (auto& entity1 : m_entityManager->GetEntityTab())
	{
		if (entity1 == nullptr)
			continue;
		if (m_entityManager->HasComponent(entity1, COMPONENT_COLLISION))
		{
			...
			for (uint32_t entity2 = entity1->tab_index + 1; entity2 < 64000; entity2++)
			{
				...
				if (m_entityManager->HasComponent(m_entityManager->GetEntityTab()[entity2], COMPONENT_COLLISION))
				{
					...
					if (AABBIntersect(*transform1, *transform2))
					{
						/*if (entity1->id == 1)
						{
							transform1->m_transform.vPosition = { 10.f, 10.f, 10.f };
						}*/
